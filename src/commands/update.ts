...
export function getReleaseArchiveName(platform: {
	...
	return platform.os === "darwin" || platform.os === "windows"
		? `${baseName}.zip`
		: `${baseName}.tar.gz`;
}

export function getBinaryName(platform: { os: string; arch: string }): string {
	return platform.os === "windows" ? "polar.exe" : "polar";
}
...
const downloadAndUpdate = (
	...
	Effect.gen(function* () {
		...
		const platform = `${platformInfo.os}-${platformInfo.arch}`;
		const archiveName = getReleaseArchiveName(platformInfo);
